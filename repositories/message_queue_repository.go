...
func (r *MessageQueueRepository) FindByStatusAndNextRetryAndRetryCountAndIsDLQ(status string, nextRetry time.Time, retryCount int, isDLQ bool) ([]models.MessageQueue, error) {
	err := r.DB.Table(models.MessageQueue.TableName(models.MessageQueue{})).Limit(10).Where("status = ? AND next_retry < ? AND retry_count < ? AND is_dlq = ?", status, nextRetry, retryCount, isDLQ).Find(&messages).Error
	if err != nil {
		return messages, err
	}
	return messages, nil
